...
(* TODO: allow to specify sampling budget *)
...
let llama_state = ref None

let make_llama_state () =
  Llama_cpp.log_set (fun _ _ -> ()) ;
  match !model_file with
  | None ->
    Printf.eprintf "%s: error: -model argument was not given\n" __FUNCTION__ ;
    exit 1
  | Some m ->
    let m =
...
let get_llama_state () =
  match !llama_state with
  | None ->
    let state = make_llama_state () in
    llama_state := (Some state) ;
    state
  | Some state ->
    state

let reset_llama_state () =
  Option.iter (fun state -> Llama_cpp.free state.Llm.ctx) !llama_state ;
  Option.iter (fun model -> Llama_cpp.free_model model) !model ;
  llama_state := None ;
  model := None
...
let () =
  LTerm_read_line.bind
    [ { control = true; meta = false; shift = false; code = Char(Uchar.of_char 'x') } ;
      { control = true; meta = false; shift = false; code = Char(Uchar.of_char 'l') } ]
    [ Edit (LTerm_edit.Zed Zed_edit.(Insert_str (Zed_string.of_utf8 "#llama-go"))) ];
...
class read_phrase ~term =
  let llama_mode = ref false in
  let rec sampling_loop print budget prev_state seq : Llm.state Lwt.t =
    if not !llama_mode || budget <= 0 then
      ...
      | Some ((token, state), rest) ->
        let output = Llm.token_to_string state token in
        let* () = Lwt.pause () in
        print output ;
        sampling_loop print (budget - 1) state rest
  in
  object(self)
    ...
    method! exec ?(keys=[]) actions =
      match actions with
      | Break :: rest when !llama_mode ->
        llama_mode := false ;
        super_term#exec rest
      | ([LTerm_read_line.Edit (LTerm_edit.Zed (Zed_edit.Insert_str str))])
        when Zed_string.compare str (Zed_string.of_utf8 "#llama-go") = 0
        ->
        llama_mode := true ;
        let to_complete = self#input_prev in
        let prefix = Zed_rope.to_string to_complete |> Zed_string.to_utf8 in
        let add_bos = Option.is_none !llama_state in
        let state = get_llama_state () in
        let state = Llm.perform_inference_on_history state in
        ...
        let print msg =
          self#send_action
            (LTerm_read_line.Edit (LTerm_edit.Zed (Zed_edit.Insert_str (Zed_string.of_utf8 msg))))
        in
        let () = Lwt.async @@ fun () ->
          sampling_loop print sampling_budget state samples
          |> Lwt.map ignore
        in
        super_term#exec []
...
let welcome term =
  ...
  let message = Printf.sprintf "Welcome to utopilot version %s (using OCaml version %s)!" UTop.version Sys.ocaml_version in
...
let args = Arg.align [
    ...
    "-principal", Arg.Set Clflags.principal, " Check principality of type inference";
      ...
      "-model", Arg.String (fun file -> model_file := Some file), " Model file for llama-mode";
  ]
